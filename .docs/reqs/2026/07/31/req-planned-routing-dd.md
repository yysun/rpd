...
- [x] The planned-routing terminus is `SS(+CR*) → TT → ET? → VR* → DD`, with `DD`
      running only after verified completion.
- [x] Planned routing still stops before `GC`; committing requires explicit `RPD` or `GC` intent.
- [x] `!!` reconciles and re-executes the current story through `VR* → DD`, then stops without
      committing or pushing.
- [x] When `!!` restarts a story with an existing matching completion document, that document stays
...
- [x] An incomplete or blocked planned route does not write a misleading completion document.
- [x] Direct routing remains unchanged: it stops after `CR` and creates no RPD artifacts.
- [x] `skills/rpd/SKILL.md`, `README.md`, the workflow diagram, and static contract coverage agree
      on the new planned-route and `!!` termini.
- [x] At least one agent-execution scenario for an ordinary planned implementation requires a
      scoped completion document and unchanged Git history.
- [x] Agent-created Markdown E2E specs use `## Scenario` sections with one or more non-empty Given,
      ...
      steps.
- [x] Checkbox-only AP progress recorded after CR for later `TT` or `ET` work does not invalidate
      CR when task text, order, scope, and all other plan content remain unchanged; substantive AP
      edits still require CR to rerun.
- [x] The skill version and changelog identify the behavior change, and repository validation
      passes.
